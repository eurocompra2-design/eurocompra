(function(){
  ...
  function init(){
    ...
    function buscar(){
      ...
      const online='https://www.google.com/search?q='+encodeURIComponent(q+' loja Bélgica');
      resultados.style.display='block';
      resultados.innerHTML=(cadastradas.length?'<strong>🔎 Loja encontrada no EuroCompra</strong>'+cadastradas.map(x=>'<a href="'+x.url+'" target="_blank" rel="noopener noreferrer" style="display:block;padding:11px 0;border-bottom:1px solid #e2e7f0;color:#063b9e;font-weight:700">'+esc(x.nome)+' <span style="font-weight:400;color:#667085">· '+esc(x.categoria)+'</span> ↗</a>').join(''):'<strong>🌐 Busca online</strong><p style="color:#667085;margin:6px 0 10px">Não encontramos esta loja na lista da EuroCompra.</p>')+'<a href="'+online+'" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;justify-content:center;background:#063b9e;color:#fff;border-radius:12px;padding:12px 16px;font-weight:800;text-decoration:none;margin-top:8px">🔗 Ver resultados online</a>';
    }
