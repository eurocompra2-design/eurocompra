(function(){
  ...
  const resultado=(nome,url)=>'<div style="padding:14px 0;border-bottom:1px solid #e2e7f0"><div style="font-size:17px;font-weight:800;color:#06245c">'+esc(nome)+'</div><div style="font-size:13px;color:#667085;margin:3px 0 10px">Site oficial</div><a href="'+url+'" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;justify-content:center;background:#063b9e;color:#fff;border-radius:10px;padding:10px 14px;font-weight:800;text-decoration:none">🌐 Abrir site oficial</a></div>';
  function init(){
    ...
    if(!box.querySelector('[data-categoria="doces"]')){
      const card=document.createElement('div');card.className='loja-card';card.dataset.categoria='doces';card.innerHTML='<h3>🍫 Doces</h3><p>Chocolaterias e lojas de chocolate na Bélgica.</p>';box.appendChild(card);
      card.addEventListener('click',function(){
        const old=box.querySelector('[data-doces="1"]');if(old)old.remove();
        const lista=document.createElement('div');lista.className='lojas-lista-aberta ativa';lista.dataset.doces='1';
        lista.innerHTML='<strong>🍫 Doces — lojas de chocolate na Bélgica</strong>'+Object.entries(chocolates).map(([nome,url])=>resultado(nome,url)).join('');box.appendChild(lista);
      });
    }
